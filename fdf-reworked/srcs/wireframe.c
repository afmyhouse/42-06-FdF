...
void	world_to_aligned(t_view *view)
{
	...
	while (++y < view->height)
	{
		...
		while (++x < view->width)
		{
			vec_mat_mult(view->points[y][x]->world, global, view->points[y][x]->aligned);
			view->points[y][x]->aligned->z = view->project ? view->points[y][x]->aligned->z : view->points[y][x]->local->z;
		}
	}
	if (MY_DEBUG)
		printf("%s(X)%s %s%s\n", SYLW, SGRN, __func__, SWHT);
