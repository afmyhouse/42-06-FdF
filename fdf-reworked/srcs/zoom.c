...
/*   Updated: 2023/10/26 21:44:22 by antoda-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	printf("%s(>)%s %s%s\n", SYLW, SYLW, __func__, SWHT);
	v->x_scale += v->keys->x * v->keys->plus / 10;
	v->x_scale -= v->keys->x * v->keys->minus / 10;
	v->y_scale += v->keys->y * v->keys->plus / 10;
	v->y_scale -= v->keys->y * v->keys->minus / 10;
	v->z_scale += v->keys->z * v->keys->plus / 10;
	v->z_scale -= v->keys->z * v->keys->minus / 10;
	printf("%s(X)%s %s%s\n", SYLW, SGRN, __func__, SWHT);
	zoom_hook(v);
	full_redraw(v);
