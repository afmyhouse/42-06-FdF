...
/*   Updated: 2023/09/18 22:00:37 by antoda-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	mlx_line_tester(t_win_data *img)
{
	...
	// diagonals
	mlx_line_r(img, &((t_mlx_line){0, F_SY, F_SX, 0, BLUE}));
	mlx_line_r(img, &((t_mlx_line){0, 0, F_SX, F_SY, BLUE}));

	mlx_line_r(img, &((t_mlx_line){F_SX / 2, 0, F_SX, F_SY, RED}));
	mlx_line_r(img, &((t_mlx_line){F_SX / 2, 0, 0, F_SY, RED}));

	mlx_line_r(img, &((t_mlx_line){F_SX / 2, F_SY, F_SX, 0, GREEN}));
	mlx_line_r(img, &((t_mlx_line){F_SX / 2, F_SY, 0, 0, GREEN}));
...
int	main(void)
{
	...
	printf(" => Window1 %dx%d \"Hello World! FHD\" ...", F_SX, F_SY);
