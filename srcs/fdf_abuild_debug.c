...
int	args_build(t_flags *f)
{
	...
	_DEBUG_ ? ft_printf("  %s%s (01) MALLOC f->cmx : %d x rows  %s\n",__func__, SCYN, f->r, SWHT) : 0;
	if (!f->cmx)
	{
		_DEBUG_ ? ft_printf("  %s%s (21) MALLOC ERROR!%s\n", __func__, SCYN, SWHT):0;
		_DEBUG_ ? ft_printf("  %s%s (22) free (f->cmx)!%s\n", __func__, SCYN, SWHT):0;
		free (f->cmx);
		_DEBUG_ ? ft_printf("  %s%s (23) free (f->line)!%s\n", __func__, SCYN, SWHT):0;
		free (f->line);
		_DEBUG_ ? ft_printf("%s%s (X2) MALLOC ERROR!%s\n", __func__, SRED, SWHT):0;
		return (MALLOC_ERROR);
	}
	f->cmx[f->r] = NULL;
	...
	while (row < f->r)
	{
		...
		_DEBUG_ ? ft_printf("  %s%s (12) MALLOC f->cmx[%d] = %s %s\n", __func__, SCYN, row, f->cmx[row], SWHT) : 0;
		ft_free_str(&f->line);
		row++;
	}
	f->line = NULL;
...
t_flags	*flags_init(int ac, char **av)
{
	...
	if (!f)
	{
		_DEBUG_ ? ft_printf("%s(X) %s MALLOC ERROR!%s\n",SYLW, SRED, __func__, SWHT):0;
		return (NULL);
	}
	_DEBUG_ ? ft_printf("  %s(1)%s %s MALLOC SUCCESS!%s\n",SYLW, SCYN, __func__, SWHT):0;
...
{
	_DEBUG_ ? ft_printf("%s(>)%s %s%s\n",SYLW, SYLW, __func__, SWHT):0;
	_DEBUG_ ? ft_printf("  %s%s (01) f->columns\t= %d%s\n", __func__, SCYN, f->c, SWHT):0;
	_DEBUG_ ? ft_printf("  %s%s (02) f->rows\t= %d%s\n", __func__, SCYN, f->r,SWHT):0;
	_DEBUG_ ? ft_printf("  %s%s (03) f->file\t= %s%s\n", __func__, SCYN, f->file,SWHT):0;
	_DEBUG_ ? ft_printf("  %s%s (04) f->fd\t\t= %d%s\n", __func__, SCYN, f->fd,SWHT):0;
	_DEBUG_ ? ft_printf("  %s%s (05) f->aux01\t= %d%s\n", __func__, SCYN, f->aux01,SWHT):0;
	_DEBUG_ ? ft_printf("  %s%s (06) f->len\t= %d%s\n", __func__, SCYN, f->len,SWHT):0;
	if (f->line)
		_DEBUG_ ? ft_printf("  %s%s (0) f->line \t=[ %p ]%s\n", __func__, SCYN, f->line,SWHT):0;
	_DEBUG_ ? ft_printf("  %s%s (07) f->cmx\t= %p%s\n", __func__, SCYN, f->cmx,SWHT):0;
	// if (f->cmx)
	// 	_DEBUG_ ? ft_printf("  %s%s (08) f->cmx\t= %s%s\n", __func__, SCYN, *(f->cmx),SWHT):0;
	_DEBUG_ ? ft_printf("  %s%s (09) f->imx\t= %p%s\n", __func__, SCYN, f->imx,SWHT):0;
	_DEBUG_ ? ft_printf("  %s%s (10) f->fmx\t= %p%s\n", __func__, SCYN, f->fmx,SWHT):0;
	_DEBUG_ ? ft_printf("%s(X)%s %s%s\n",SYLW, SGRN, __func__, SWHT):0;
	return ;
}
...
int	args_check(t_flags *f)
{
	...
	_DEBUG_ ? ft_printf("%s(>)%s %s%s\n",SYLW, SYLW, __func__, SWHT):0;
	f->fd = open(f->file, O_RDONLY);
	_DEBUG_ ? ft_printf("  %s%s (01) f->fd = %d%s\n", __func__, SCYN, f->fd, SWHT):0;
	if (f->fd == -1)
	{
		_DEBUG_ ? ft_printf("%s(X1)%s %s FILE NOT FOUND!%s\n",SYLW, SRED, __func__, SWHT):0;
		return (FILE_NOT_FOUND);
	}
	f->line = get_next_line(f->fd);
	if (!f->line)
	{
		_DEBUG_ ? ft_printf("%s(X1)%s %s FILE IS EMPTY!%s\n",SYLW, SRED, __func__, SWHT):0;
		return (FILE_EMPTY);
	}
	_DEBUG_ ? ft_printf("  %s%s (02) malloc f->line!%s\n", __func__, SCYN, SWHT):0;
	f->c = ft_split_count(f->line, ' ');
	_DEBUG_ ? ft_printf("  %s%s (03) f->c = %d%s\n", __func__, SCYN, f->c, SWHT):0;
	while (f->line)
	{
		_DEBUG_ ? ft_printf("\n  %s%s (10) while(%p) %s\n", __func__, SCYN, f->line, SWHT):0;
		f->r++;
		...
		_DEBUG_ ? ft_printf("  %s%s (11) columns = %d, line = %s!%s\n", __func__, SCYN, columns, f->line, SWHT):0;
		ft_free_str(&f->line);
		_DEBUG_ ? ft_printf("  %s%s (12) free f->line!%s\n", __func__, SCYN, SWHT):0;
		if (columns != f->c)
		{
			_DEBUG_ ? ft_printf("  %s%s (21) columns = %i != f->c = %i%s\n", __func__, SCYN, columns, f->c, SWHT):0;
			_DEBUG_ ? ft_printf("%s%s (X3) INVALID MAP!%s\n", __func__, SRED, SWHT):0;
			return (MAP_ERROR);
		}
		f->line = get_next_line(f->fd);
		if (f->line)
		{
			_DEBUG_ ? ft_printf("  %s%s (31) malloc f->line!%s\n", __func__, SCYN,SWHT):0;
			//_DEBUG_ ? ft_printf("%s%s (32) inside line = %s!%s\n",  __func__, SCYN, f->line, SWHT):0;
		}
	}
	close(f->fd);
	if(f->line)
	{
		ft_free(f->line);
	_DEBUG_ ? ft_printf("  %s%s (41) free line!%s\n", __func__, SCYN, SWHT):0;
	}
		_DEBUG_ ? ft_printf("  %s%s (51) Matrix size : Row(s) = %d, Column(s) = %d%s\n", __func__, SCYN, f->r, f->c, SWHT):0;
	_DEBUG_ ? ft_printf("%s(X)%s %s%s\n",SYLW, SGRN, __func__, SWHT):0;
	return (SUCCESS);
}
