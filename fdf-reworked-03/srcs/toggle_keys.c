...
/*   Updated: 2023/10/28 17:19:58 by antoda-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	keys_scale_status(int kc, t_view *v, int kt)
{
	if (MY_DEBUG)
		printf("%s(>)%s %s =>  kc = %c(%#x), Toggle = %d%s\n", SYLW, SYLW, __func__,  kc,  kc, kt, SWHT);
...
void	keys_zoom_status(int kc, t_view *v, int kt)
{
	if (MY_DEBUG)
		printf("%s(>)%s %s => kc = %c(%#x), Toggle = %d%s\n", SYLW, SYLW, __func__, kc, kc, kt, SWHT);
...
void	toggle_key(int kc, t_view *v, int kt)
{
	if (MY_DEBUG)
		printf("%s(>)%s %s => kc = %c(%#x), Toggle = %d%s\n", SYLW, SYLW, __func__, kc, kc, kt, SWHT);
